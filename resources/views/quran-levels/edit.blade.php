...
@section('title', __('quran_levels.edit_title'))

@section('content')
<div class="page-wrapper">
    <div class="container mt-5">
        <h2>{{ __('quran_levels.edit_title') }}</h2>
        ...
        <form action="{{ route('quran-levels.update', $quranLevel->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-3">
                ...
                <select 
                    ...
                    required 
                    onchange="filterAcademicYears()"
                >
                    <option value="">{{ __('quran_levels.select_school') }}</option>
                    @foreach($schools as $school)
                        <option value="{{ $school->id }}" {{ old('school_id', $quranLevel->school_id) == $school->id ? 'selected' : '' }}>
                            {{ $school->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                >
                    ...
                    @foreach($academicYears as $year)
                        <option 
                            value="{{ $year->id }}" 
                            data-school="{{ $year->school_id }}" 
                            {{ old('academic_year_id', $quranLevel->academic_year_id) == $year->id ? 'selected' : '' }}
                        >
                            {{ $year->name }}
                        </option>
                    @endforeach
            ...
            <div class="mb-3">
                ...
                <input 
                    ...
                    required 
                    value="{{ old('name', $quranLevel->name) }}"
                >
                @error('name')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            ...
            <div class="mb-3">
                ...
                <input 
                    ...
                    name="level_order" 
                    id="level_order" 
                    class="form-control @error('level_order') is-invalid @enderror" 
                    required 
                    value="{{ old('level_order', $quranLevel->level_order) }}"
                >
                @error('level_order')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            ...
            <div class="mb-3">
                ...
                >{{ old('description', $quranLevel->description) }}</textarea>
                @error('description')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const selectedYearId = "{{ old('academic_year_id', $quranLevel->academic_year_id) }}";
        if (selectedYearId) {
